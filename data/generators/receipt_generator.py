...
def create_standard_receipt(width, height, items_count=None):
    ...
    draw.line([(margin, current_y), (width - margin, current_y)], fill="black", width=1)
    current_y += 15
    ...
    draw.text(
        (margin + date_text_width + 10, current_y),
        current_date,
        fill="black",
        font=font_body,
    )
    ...
    draw.text(
        (margin + time_text_width + 10, current_y),
        current_time,
        fill="black",
        font=font_body,
    )
    ...
    draw.text(
        (receipt_number_x, current_y), receipt_number, fill="black", font=font_body
    )
    ...
    for (
        ...
        price_range,
        _,
    ) in selected_items:  # use _ since we don't use the unit variable
        ...
        draw.text(
            (qty_center - qty_width // 2, current_y),
            qty_str,
            fill="black",
            font=font_body,
        )
        ...
        draw.text(
            (price_x + 50 - price_width, current_y),
            price_str,
            fill="black",
            font=font_body,
        )
        draw.text(
            (total_x + 50 - total_width, current_y),
            total_str,
            fill="black",
            font=font_body,
        )
    ...
    draw.text(
        (width - margin - subtotal_width, current_y),
        subtotal_str,
        fill="black",
        font=font_body,
    )
    ...
    draw.text(
        (width - margin - tax_width, current_y), tax_str, fill="black", font=font_body
    )
    ...
    draw.text(
        (width - margin - total_width, current_y),
        total_str,
        fill="black",
        font=font_subheader,
    )
    ...
    draw.line(
        [(margin, footer_y - 15), (width - margin, footer_y - 15)],
        fill="black",
        width=1,
    )
...
def create_detailed_receipt(width, height, items_count=None):
    ...
    draw.rectangle(
        [(0, 0), (width, header_height)], fill=(240, 240, 240)
    ...
    date_text_width = font_body.getlength("Date:")
    draw.text(
        (left_col_x + date_text_width + 10, current_y),
        current_date,
        fill="black",
        font=font_body,
    )
    ...
    time_text_width = font_body.getlength("Time:")
    draw.text(
        (left_col_x + time_text_width + 10, current_y),
        current_time,
        fill="black",
        font=font_body,
    )
    ...
    draw.text(
        (right_col_x + receipt_text_width + 10, detail_start_y),
        receipt_number,
        fill="black",
        font=font_body,
    )
    ...
    draw.text(
        (right_col_x, detail_start_y + font_body.size + 10),
        f"Served by: {staff_name} (Reg {register_num})",
        fill="black",
        font=font_body,
    )
    ...
    for i, (name, price_range, unit) in enumerate(selected_items):
        ...
        draw.text(
            (price_x + 60 - price_width, current_y),
            price_str,
            fill="black",
            font=font_body,
        )
        draw.text(
            (total_x + 60 - total_width, current_y),
            total_str,
            fill="black",
            font=font_body,
        )
    ...
    total_bg_padding = 10
    total_text_width = (
        font_subheader.getlength("TOTAL:") + 10 + font_subheader.getlength(total_str)
    )
    ...
    draw.rectangle(
        [(total_bg_left, total_bg_top), (total_bg_right, total_bg_bottom)],
        fill=(240, 240, 240),
    )  # Light gray background
    ...
    draw.text(
        (payment_header_x, payment_header_y),
        payment_header,
        fill="black",
        font=font_body,
    )
    ...
    draw.text(
        (width - margin - amount_width - 10, payment_details_y),
        amount_text,
        fill="black",
        font=font_body,
    )
    ...
    draw.text(
        (gst_x, footer_y + font_body.size + 10),
        gst_message,
        fill="black",
        font=font_small,
    )
...
def create_fancy_receipt(width, height, items_count=None):
    ...
    draw.line(
        [(margin, title_line_y), (title_line_left, title_line_y)],
        fill=(100, 100, 100),
        width=1,
    )
    draw.line(
        [(title_line_right, title_line_y), (width - margin, title_line_y)],
        fill=(100, 100, 100),
        width=1,
    )
    ...
    draw.line(
        [(margin, current_y), (width - margin, current_y)],
        fill=(150, 150, 150),
        width=1,
    )
    ...
    for i in range(margin, width - margin, 10):
        draw.line(
            [(i, current_y + 5), (i + 5, current_y + 5)], fill=(200, 200, 200), width=1
        )
    ...
    date_text_width = font_body.getlength("Date:")
    draw.text(
        (detail_x + date_text_width + 10, detail_y),
        current_date,
        fill="black",
        font=font_body,
    )
    ...
    time_text_width = font_body.getlength("Time:")
    draw.text(
        (detail_x + time_text_width + 10, detail_y),
        current_time,
        fill="black",
        font=font_body,
    )
    ...
    receipt_text_width = font_body.getlength("Receipt:")
    draw.text(
        (right_detail_x + receipt_text_width + 10, detail_y),
        receipt_number,
        fill="black",
        font=font_body,
    )
    ...
    detail_y += font_body.size + 10
    draw.text(
        (right_detail_x, detail_y),
        f"Served by: {staff_name}",
        fill="black",
        font=font_body,
    )
    ...
    draw.text(
        (item_header_x, current_y), item_header, fill="black", font=font_subheader
    )
    ...
    draw.rectangle(
        [(table_left, header_top), (table_right, header_top + header_height)],
        fill=(240, 240, 240),
    )
    ...
    draw.line(
        [(table_left, current_y + 5), (table_right, current_y + 5)],
        fill=(150, 150, 150),
        width=1,
    )
    current_y += 15
    ...
    subtotal_width = font_body.getlength(subtotal_str)
    draw.text(
        (table_right - subtotal_width - 10, current_y),
        subtotal_str,
        fill="black",
        font=font_body,
    )
    ...
    tax_width = font_body.getlength(tax_str)
    draw.text(
        (table_right - tax_width - 10, current_y), tax_str, fill="black", font=font_body
    )

    # Add separator before total
    current_y += item_height - 5
    draw.line(
        [(totals_left, current_y), (table_right, current_y)],
        fill=(100, 100, 100),
        width=1,
    )
    current_y += 10
    ...
    total_width = font_subheader.getlength(total_str)
    draw.text(
        (table_right - total_width - 10, current_y),
        total_str,
        fill="black",
        font=font_subheader,
    )
    ...
    draw.text(
        (payment_x, payment_box_top + 20), payment_text, fill="black", font=font_body
    )
    ...
    for i in range(margin, width - margin, 10):
        draw.line(
            [(i, footer_y - 15), (i + 5, footer_y - 15)], fill=(200, 200, 200), width=1
        )
    ...
    draw.text(
        (message_x, footer_y + font_body.size + 10),
        custom_message,
        fill="black",
        font=font_small,
    )
...
def create_minimal_receipt(width, height, items_count=None):
    ...
    draw.text(
        (margin, current_y), f"Date: {current_date}", fill="black", font=font_body
    )
    current_y += font_body.size + 10
    draw.text(
        (margin, current_y), f"Time: {current_time}", fill="black", font=font_body
    )
    ...
    draw.text(
        (margin, current_y), f"Receipt: {receipt_id}", fill="black", font=font_body
    )
    ...
    for name, price_range, _ in selected_items:
        ...
        total_width = font_body.getlength(total_str)
        price_x = (
            width - margin - price_column_width + 30
...
def create_receipt(image_size=2048):
    ...
    scale_factor = min(
        image_size / 2 / receipt.width, image_size / 1.2 / receipt.height
    )
...
def generate_test_dataset(output_dir, num_receipts=20, image_size=2048, seed=42):
    ...
    for i in range(num_receipts):
        ...
        try:
            ...
            for name, price_range, _ in selected_items:
                ...
                price_variation = random.uniform(0.95, 1.05)
                price = (
                    round(base_price * price_variation) / 100.0
                )  # Convert to dollars
    ...
    print(f"Store distribution: {metadata_df['store_name_value'].value_counts().head()}")
    print(f"Average items per receipt: {metadata_df['items_count'].mean():.1f}")

    # Print directory structure
    print("\nTest directory structure:")
    print(f"- Images: {test_img_dir} ({len(list(test_img_dir.iterdir()))} files)")
    print(
        f"- Ground truth: {test_ground_truth_dir} ({len(list(test_ground_truth_dir.iterdir()))} files)"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate Australian receipts for information extraction testing"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    print(
        f"Generating {args.num_receipts} Australian receipts for information extraction testing..."
    )
    dataset_df = generate_test_dataset(
    ...
    print(f"Generated {len(dataset_df)} receipts in {args.output_dir}/test/")
    print("\nDirectory structure:")
    ...
    print(
        f"python generate_predictions.ipynb with INTERNVL_IMAGE_FOLDER_PATH set to {args.output_dir}/test/images"
    )
