...
def generate_receipt(output_dir, ground_truth_dir, include_gst=True, receipt_id=None):
    ...
    draw.text(
        (500, y_pos), f"${subtotal:.2f}", fill="black", font=normal_font, weight="bold"
    )
    ...
    image_path = output_path / f"{receipt_id}.jpg"
    receipt.save(str(image_path))
    ...
    if ground_truth_dir:
        ...
        ground_truth_path = gt_path / f"{receipt_id}.json"
...
def setup_test_directories(base_dir):
    ...
    ground_truth_dir = base_path / "ground_truth"
    ...
    return {
        ...
        "test_ground_truth_dir": str(ground_truth_dir),
    }


def main():
    parser = argparse.ArgumentParser(
        description="Generate synthetic Australian receipts for testing only"
    )
    parser.add_argument(
        "--base_dir",
        type=str,
    ...
    parser.add_argument(
        "--include_gst", action="store_true", help="Include GST (Australian tax)"
    )
    ...
    print(
        f"Generating {args.num_receipts} Australian receipts for testing with GST: {args.include_gst}"
    )
