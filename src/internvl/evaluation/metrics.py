...
from src.internvl.utils import get_logger
...
def calculate_metrics(actual: str, predicted: str, image_id: str) -> Dict[str, float]:
    ...
    common = actual_chars & predicted_chars  # Intersection (common characters with counts)
    tp = sum(common.values())  # True Positives (correctly predicted characters)
    fp = sum(predicted_chars.values()) - tp  # False Positives (extra predicted characters)
    ...
    bleu = sentence_bleu([list(actual)], list(predicted), 
...
def get_average_metrics_per_field(
    ...
    for field in evaluation_fields:
        ...
        predicted_output = [" ".join(str(x) for x in item) if isinstance(item, list) else str(item) for item in predictions[field]]
        ground_truth = [" ".join(str(x) for x in item) if isinstance(item, list) else str(item) for item in actuals[field]]
...
def get_average_metrics_per_parsed_fields(
    ...
    for field in evaluation_fields:
        if field == 'date_value':
            ...
            predicted_output = [" ".join(normalize_evaluation_date(str(x)) for x in item) if isinstance(item, list) else normalize_evaluation_date(str(item)) for item in predictions[field]]
            ground_truth = [" ".join(normalize_evaluation_date(str(x)) for x in item) if isinstance(item, list) else normalize_evaluation_date(str(item)) for item in actuals[field]]
        elif field in ["prod_price_value", "tax_value", "total_value"]:
            # Clean numeric values for proper comparison
            predicted_output = [" ".join(clean_numeric(str(x)) for x in item) if isinstance(item, list) else clean_numeric(str(item)) for item in predictions[field]]
            ground_truth = [" ".join(clean_numeric(str(x)) for x in item) if isinstance(item, list) else clean_numeric(str(item)) for item in actuals[field]]
        else:
...
def normalize_field_values(data_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "date_value" in normalized:
        normalized["date_value"] = normalize_evaluation_date(str(normalized["date_value"]))
    
    if "store_name_value" in normalized:
        normalized["store_name_value"] = normalize_store_name(str(normalized["store_name_value"]))
    ...
    # Handle list fields
    if "prod_item_value" in normalized and isinstance(normalized["prod_item_value"], list):
        normalized["prod_item_value"] = normalize_list_field(normalized["prod_item_value"], "items")
    
    if "prod_quantity_value" in normalized and isinstance(normalized["prod_quantity_value"], list):
        normalized["prod_quantity_value"] = normalize_list_field(normalized["prod_quantity_value"], "quantities")
    
    if "prod_price_value" in normalized and isinstance(normalized["prod_price_value"], list):
        normalized["prod_price_value"] = normalize_list_field(normalized["prod_price_value"], "prices")
...
def calculate_field_metrics(
    ...
    if len(ground_truth_dict) > 0 and "tax_value" in evaluation_fields and "total_value" in evaluation_fields:
        ...
        if tax_validation_metrics:
            ...
            for metric in ["Precision", "Recall", "F1-score", "BLEU Score"]:
                ...
                updated_values[metric] = sum(all_values) / len(all_values) if all_values else 0
            overall = updated_values
    
    return overall, field_metrics

def validate_gst_calculation(results: List[Dict], ground_truth: Dict) -> Dict[str, float]:
    """
    ...
    for result in results:
        ...
        try:
            pred_tax = float(re.sub(r'[^\d.]', '', str(prediction.get("tax_value", ""))))
            pred_total = float(re.sub(r'[^\d.]', '', str(prediction.get("total_value", ""))))
            gt_tax = float(re.sub(r'[^\d.]', '', str(gt.get("tax_value", ""))))
