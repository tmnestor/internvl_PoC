...
def setup_transformers_logging(level: Union[int, str] = logging.INFO) -> None:
    """
    ...
    Args:
        level: Logging level to set for transformers (e.g., logging.WARNING to hide INFO messages)
    """
    try:
        ...
        # Always set transformers logging to WARNING to suppress INFO messages like
        # "Setting `pad_token_id` to `eos_token_id`:151645 for open-end generation."
        transformers_logging.set_verbosity(logging.WARNING)
        logger = logging.getLogger(__name__)
        logger.debug(f"Transformers logging level set to: {logging.getLevelName(logging.WARNING)}")
    except ImportError:
