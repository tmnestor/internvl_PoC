...
from src.internvl.config.config import load_config
from src.internvl.evaluation.metrics import calculate_field_metrics
from src.internvl.utils.logging import get_logger, setup_logging
from src.internvl.utils.path import PathManager
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--normalize",
        help="Normalize fields before comparison",
        action="store_true"
    )
    parser.add_argument(
        "--verbose",
...
def run_evaluation(
    ...
    if "gst_calculation" in field_metrics:
        ...
        logger.info("- In Australia, GST is 10% of the pre-tax amount (1/11 of the total)")
    ...
    if show_examples and examples:
        ...
        for image_id, data in examples.items():
            ...
            # Compare each field
            for field in (fields or evaluation_fields):
                ...
                if field == "date_value":
                    logger.info("  Note: Check date format - Australian format is DD/MM/YYYY")
                elif field == "tax_value":
                    # Calculate expected GST
                    try:
                        total = float(re.sub(r'[^\d.]', '', str(data["ground_truth"].get("total_value", ""))))
                        expected_gst = round(total / 11, 2)  # GST is 1/11 of total in Australia
                        logger.info(f"  Expected GST (1/11 of total): ${expected_gst:.2f}")
...
def main():
    ...
    # Resolve paths
    predictions_dir = Path(args.predictions_dir) if args.predictions_dir else path_manager.get_output_path("predictions_test")
    ground_truth_dir = Path(args.ground_truth_dir) if args.ground_truth_dir else path_manager.get_synthetic_ground_truth_path()
    output_path = Path(args.output_path) if args.output_path else path_manager.get_output_path("evaluation_results")
    ...
    try:
        ...
        if f1_score < 0.9:
            ...
            if args.save_prompt_template:
                prompt_file = path_manager.get_source_path() / "australian_receipt_prompt.txt"
        ...
        if 'scalar_metrics' in results and 'list_metrics' in results:
            print(f"Scalar fields F1-score (date, store, tax, total): {results['scalar_metrics'].get('F1-score', 0):.2%}")
            print(f"List fields F1-score (products, quantities, prices): {results['list_metrics'].get('F1-score', 0):.2%}")
